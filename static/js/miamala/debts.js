...
function fill_edit_form(rowIndex, id, str) {
  if (str == "edit") {
    var row = $("#debts_table tbody tr:nth-child(" + (rowIndex + 1) + ")");
    var names = $("td:nth-child(3)", row).text();
    var debt = $("td:nth-child(6)", row).text().replace(/,/g, "");
    var describe = $("td:nth-child(1)", row).attr("data-bs-describe");
    describe = describe == "null" ? "" : describe;
    $("#debt_edit_names").val(names);
    $("#debt_edit_amount").val(parseFloat(debt));
    $("#debt_edit_description").val(describe);
    $("#debt_id").val(id);
    $("#update_debt_modal").modal("show");
  } else {
    $("#debt_del_id").val(id);
    $("#delete_debt_modal").modal("show");
  }
}

// check debt numbers
function check_debt(debt, paid) {
  var status = false;
  if (paid < 0) {
    if (Math.abs(paid) <= debt) {
      status = true;
    }
  } else {
    status = true;
...
$(function () {
  ...
  // New debt
  $("#new_debt_form").submit(function (e) {
    e.preventDefault();
    var debt_amount = $("#debt_amount").val();
    var formSms = $("#new_debt_form .formsms");
    var debt_names = $.trim($("#debt_names").val());
    var debt_describe = $.trim($("#debt_description").val());
    var formData = new FormData();
    formData.append("names", debt_names);
    formData.append("amount", debt_amount);
    formData.append("describe", debt_describe);

    var submitBtn = $("#new_debt_btn");
    $.ajax({
      type: "POST",
  ...
  // submit debt update form
  $("#edit_debt_form").submit(function (e) {
    e.preventDefault();
    var formSms = $("#edit_debt_form .formsms");
    var submitBtn = $("#debt_edit_btn");

    var debt_names = $.trim($("#debt_edit_names").val());
    var debt_amount = parseFloat($("#debt_edit_amount").val());
    var debt_paid = parseFloat($("#debt_edit_paid").val());
    var debt_describe = $.trim($("#debt_edit_description").val());

    if (check_debt(debt_amount, debt_paid)) {
      var formData = new FormData();
      formData.append("debt_id", $("#debt_id").val());
      formData.append("names", debt_names);
      formData.append("paid", debt_paid);
      formData.append("describe", debt_describe);
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          var alert;
          if (response.success) {
            $("#debt_edit_amount").val(debt_amount + debt_paid);
            $("#debt_edit_paid").val("");
            alert = `<i class="fas fa-check-circle"></i> &nbsp; ${response.sms}`;
            formSms.removeClass("alert-danger").addClass("alert-success");
            table.draw();
      ...
      var alert = `<i class="fas fa-exclamation-circle"></i> &nbsp; Paid amount cannot exceed current debt!`;
      formSms
  ...
  // delete debt
  $("#del_debt_form").submit(function (e) {
    e.preventDefault();
    var del_debt_id = $("#debt_del_id").val();
    if (parseInt(del_debt_id) > 0) {
      var formData = new FormData();
      formData.append("delete_id", del_debt_id);

      var submitBtn = $("#debt_delete_btn");
      var formSms = $("#del_debt_form .formsms");
  ...
  function get_dates_range() {
    const minDateStr = $("#min_debt_date").val();
    const maxDateStr = $("#max_debt_date").val();
  ...
  // DataTable configs
  $("#debts_table thead tr")
    .clone(true)
    .attr("class", "filters")
    .appendTo("#debts_table thead");

  var table = $("#debts_table").DataTable({
    fixedHeader: true,
    processing: true,
    serverSide: true,
    ajax: {
      url: $("#debts_page_url").val(),
      type: "POST",
      data: (d) => {
  ...
  // table global search
  $("#search_debt_field").keyup(function () {
    table.search($(this).val()).draw();
  });

  // clear all table filters
  $("#debts_filter_clear").click(function (e) {
    e.preventDefault();
    $("#search_debt_field").val("");
    $("#min_trans_date").val("");
    $("#max_trans_date").val("");
  ...
  function updateFooter(totals) {
    ...
    var date_start = $("#min_debt_date").val();
    var date_end = $("#max_debt_date").val();

...
function fill_edit_form(rowIndex, id, str) {
  if (str == "edit") {
    var row = $("#selcompay_table tbody tr:nth-child(" + (rowIndex + 1) + ")");
    ...
    $("#sel_edit_names").val($("td:nth-child(3)", row).text());
    $("#sel_edit_amount").val(parseFloat(amount));
    $("#sel_edit_description").val(describe);
    $("#transaction_id").val(id);
    $("#update_selcom_modal").modal("show");
  } else {
    $("#selcom_del_id").val(id);
    $("#delete_selcom_modal").modal("show");
  }
}
...
$(function () {
  ...
  // New selcompay transactions
  $("#new_selcom_transaction_form").submit(function (e) {
    e.preventDefault();
    var formSms = $("#new_selcom_transaction_form .formsms");
    var trans_names = $.trim($("#sel_names").val());
    var trans_amount = $("#sel_amount").val();
    var trans_describe = $.trim($("#sel_description").val());
    var formData = new FormData();
    formData.append("names", trans_names);
    formData.append("amount", trans_amount);
    formData.append("describe", trans_describe);

    var submitBtn = $("#sel_trans_btn");
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.success) {
          ...
          $("#new_selcom_transaction_form")[0].reset();
          table.draw();
        } else {
  ...
  // submit transaction update form
  $("#edit_selcom_transaction_form").submit(function (e) {
    e.preventDefault();
    var formSms = $("#edit_selcom_transaction_form .formsms");
    var trans_names = $.trim($("#sel_edit_names").val());
    var trans_amount = $("#sel_edit_amount").val();
    var trans_describe = $.trim($("#sel_edit_description").val());
    var formData = new FormData();
    ...
    var submitBtn = $("#sel_edit_trans_btn");
  ...
  // delete selcompay transaction
  $("#del_selcom_form").submit(function (e) {
    e.preventDefault();
    var selcom_id = $("#selcom_del_id").val();
    if (parseInt(selcom_id) > 0) {
      var formData = new FormData();
      formData.append("delete_id", selcom_id);

      var submitBtn = $("#sel_delete_btn");
      var formSms = $("#del_selcom_form .formsms");
  ...
  // DataTable configs
  $("#selcompay_table thead tr")
    .clone(true)
    .attr("class", "filters")
    .appendTo("#selcompay_table thead");

  var table = $("#selcompay_table").DataTable({
    fixedHeader: true,
    processing: true,
    serverSide: true,
    ajax: {
      url: $("#selcom_pay_page").val(),
      type: "POST",
      data: (d) => {
  ...
  // table global search
  $("#search_selcom_field").keyup(function () {
    table.search($(this).val()).draw();
  });

  // clear all table filters
  $("#selcom_filter_clear").click(function (e) {
    e.preventDefault();
    $("#search_selcom_field").val("");
    $("#min_trans_date").val("");
    $("#max_trans_date").val("");
